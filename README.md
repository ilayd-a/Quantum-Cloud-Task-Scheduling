...
Once expanded, we can collect coefficients into a matrix \(Q\) (for all \(x_i x_j\) pairs) and a vector \(c\) (for single-variable terms), giving:
...
When we fully expand both squared terms, we group everything into this generic shape:

\[
E(x) = \sum_{i,j,m,m'} Q_{(i,m),(j,m')} x_{i,m} x_{j,m'} 
+ \sum_{i,m} c_{i,m} x_{i,m} + \text{const}
\]
...
We can think of \(Q\) as composed of two types of blocks:
...
- **Off-diagonal entries:**  
  \[
  Q_{(i,m),(j,m)} = 2 \lambda_1 p_i p_j \quad \text{for } i \neq j
  \]
...
- **Off-diagonal entries:**  
  \[
  Q_{(i,m),(i,m')} = 2\lambda_2 \quad \text{for } m \neq m'
  \]

The full \(Q\) is just these effects added together.  
Every cell in \(Q\) tells you the “energy interaction” if variable \(u\) and variable \(v\) are both 1.
...
- entries in \(Q\) become couplings \(J_{ij}\)
...
- **Cost unitaries:** controlled-phase gates for coupling terms \(J_{i,j}\) and single-qubit \(e^{-i\gamma h_i Z_i}\).  
