...
def run_single_experiment(args: argparse.Namespace, dataset_override: str | Path | None = None, output_override: str | Path | None = None, run_id: int | None = None) -> Dict[str, Any]:
    ...
    qaoa_res = solve_qaoa_local(
        ...
        top_k=getattr(args, 'top_k', 50),  # Use top_k from args, default to 50
    )
    ...
    qaoa_makespan = qaoa_schedule.get("makespan")  # Min-energy selection makespan
    min_makespan_in_samples = qaoa_res.get("min_makespan_in_samples")
    topk_makespan = qaoa_res.get("topk_makespan")  # Top-K postselection makespan
    classical_makespan = classical_res.get("makespan")

    # Relative gap for min-energy selection
    relative_gap = None
    ...
    # Relative gap for top-K postselection
    topk_gap = None
    if topk_makespan is not None and classical_makespan:
        topk_gap = 100.0 * (topk_makespan - classical_makespan) / classical_makespan
    ...
    payload: Dict[str, Any] = {
        ...
        "metrics": {
            "qaoa_makespan": qaoa_makespan,  # Min-energy selection
            "topk_makespan": topk_makespan,  # Top-K postselection
            "min_makespan_in_samples": min_makespan_in_samples,
            "classical_makespan": classical_makespan,
            "relative_gap_pct": relative_gap,  # Min-energy selection gap
            "topk_gap_pct": topk_gap,  # Top-K postselection gap
    ...
    print(
        f"Saved {output_path} | makespans (min-energy={summary['qaoa_makespan']}, top-K={summary['topk_makespan']}, classical={summary['classical_makespan']}) | gaps (min-energy={summary['relative_gap_pct']:.2f}%, top-K={summary['topk_gap_pct']:.2f}%)"
    )
                ...
                rows.append(
                    {
                        ...
                        "topk_makespan": metrics.get("topk_makespan"),
                        ...
                        "topk_gap_pct": metrics.get("topk_gap_pct"),
